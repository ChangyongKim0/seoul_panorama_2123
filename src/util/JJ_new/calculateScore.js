...
export function calculateScore(
  ...
  const region_no = parseInt(region_num);
  const developedPiljiData = getDevelopedPilji(bldg_state, background_relation);
  ...
  if (num_develop === 0 || (num_develop < 6 && e_Bldg.length === num_develop)) {
    return {
      l: 0,
      b: 0,
      p: 0,
    ...
    // 건물 나누기
    const n_bldg_raw = developedPiljiData.filter(
      (x) => x.bldg_type === "normal"
    ); // NORMAL 빌딩
    ...
    const isNBldgDeadOrAlive = (n_bldg) => {
      ...
      return n_bldg.reduce(
        (result, obj) => {
          const bldg_destiny = isAllOverlapped(obj.bldg_configuration)
            ? "dead_bldg"
            : "alive_bldg";
    ...
    const x_next_Bldg = developedPiljiData.filter(
      (x) =>
        x.bldg_type === "" && x.overlapped.some((y) => x_bldg_guid.includes(y))
    ); // x건물 옆필지

    // 개발적합도 계산 : piljiTypicalities x minAreaCoeffient
    function getDevelopmentFitness(x) {
      const min_area = 100; // 최소면적 파라미터 이거보다 낮으면 개발적합도 쭉떨어짐
      const min_area_pt = 350; // 평균 필지크기 0.5배 // 이거 이하면 필지 비정형도 고려
      if (x.area < min_area_pt) {
        return (
          Math.min(((16 * x.area) / x.perimeter ** 2),1) *
    ...
    function getsumScores(scoresWithId) {
      ...
      const lv_sum = scoresWithId.reduce(
        (sum, item) => sum + (item.lv || 0),
        0
      );
      const bv_sum = scoresWithId.reduce(
        (sum, item) => sum + (item.bv || 0),
        0
      );
      const pv_sum = scoresWithId.reduce(
        (sum, item) => sum + (item.pv || 0),
        0
      );
    ...
    const sum = getSum(sum_n_bldgscores,sum_b_bldgscores,sum_e_bldgscores,sum_x_bldgscores,sum_xnext_bldgscores,e_Bldg)
